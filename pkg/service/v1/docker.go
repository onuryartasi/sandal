...
func (s *Service) CreateProject(ctx context.Context, project *v1.Project) (*v1.ProjectInfo, error) {
	...
	for i := 0; i < min; i++ {
		...
		resp, err := cli.ContainerCreate(context.Background(), &container.Config{Image: image}, &container.HostConfig{Resources: container.Resources{CPUQuota: 30000}}, nil, fmt.Sprintf("%s%s", project.GetName(), strconv.Itoa(i+1)))
